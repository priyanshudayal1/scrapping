...
PROGRESS_FILE = os.path.join(SCRIPT_DIR, f"script54_progress.json")
TIMING_FILE = os.path.join(SCRIPT_DIR, f"script54_timing.json")

# Configure logging with UTF-8 encoding
log_file = os.path.join(SCRIPT_DIR, f"script54.log")
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - [Script 54] - %(levelname)s - %(message)s',
    handlers=[
...
def fill_captcha():
    ...
    attempt = 0
    # Unlimited retries - keep trying until captcha is solved
    while True:
        attempt += 1
        try:
            ...
            # Use AWS Bedrock (Amazon Nova) to solve captcha with fallback
...
if __name__ == "__main__":
    try:
        logger.info("=" * 80)
        logger.info(f"Starting Script 54")
        logger.info(f"Page Range: 135,628 to 138,186")
        logger.info(f"Debugging Port: {9222 + SCRIPT_ID}")
        import tempfile
